...
class GameParser {
    ...
    private fun buildOther(frame: String): Frame {
        return when {
            SCORE_TWICE_LEFT_SOME_WITHOUT_FALLING.matches(frame) -> Regular(scoreOf(frame[0]), scoreOf(frame[1]))
            SCORE_THEN_MISS.matches(frame) -> Regular(scoreOf(frame[0]), 0)
            IS_SPARE.matches(frame) -> Spare(scoreOf(frame[0]))
            IS_STRIKE.matches(frame) -> Strike
            else -> throw IllegalArgumentException("Wrong format of frame")
        }
    }

    private fun buildLast(frame: String): LastFrame = LastFrame(scoreOf(frame, 0), scoreOf(frame, 1), scoreOf(frame, 2))

    private fun scoreOf(frame: String, pos: Int): Int {
        return when {
            pos < frame.length && pos > 0 -> scoreOf(frame[pos], frame[pos - 1])
            pos < frame.length -> scoreOf(frame[pos])
            else -> 0
        }
    }

    private fun scoreOf(symbol: Char, previous: Char = '0'): Int {
        return when (symbol) {
            '-' -> 0
